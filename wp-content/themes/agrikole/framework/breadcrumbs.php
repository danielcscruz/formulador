...
function agrikole_breadcrumbs() {

    global $post;

    $home_text     = esc_html__( 'Home','agrikole' );
    $category_text = esc_html__( 'Archive by Category "%s"','agrikole' );
    $tax_text      = esc_html__( 'Archive by "%s"','agrikole' );
    $tag_text      = esc_html__( 'Posts Tagged "%s"','agrikole' );
    $author_text   = esc_html__( 'Articles Posted by %s','agrikole' );
    $error_text    = esc_html__( 'Error 404','agrikole');
    $search_text   = esc_html__( 'Search Results for "%s" Query','agrikole' );

    $blog_text     = esc_html__( 'Blog','agrikole' );
    $project_text  = esc_html__( 'Projects','agrikole' );
    $product_text  = esc_html__( 'Shop','agrikole' );
            ...
            foreach ( $cpt_list as $cpt ) {
                ...
                if ( $cpt->name == 'project' ) {
                    if ( is_singular( 'project' ) ) {
                        ...
                        if ( $page_id = agrikole_parse_obj_id( agrikole_get_mod( 'portfolio_page' ), 'page' ) ) {
