...
// Start class
if ( ! class_exists( 'Agrikole_Typography' ) ) {
	class Agrikole_Typography {
		public function __construct() {
			// Customizer actions
			add_action( 'customize_register', array( 'Agrikole_Typography' , 'register' ), 40 );

			// Loads Google fonts
			add_action( 'wp_enqueue_scripts', array( 'Agrikole_Typography', 'load_fonts' ) );
		
			// CSS output
			if ( is_customize_preview() ) {
				add_action( 'customize_preview_init', array( 'Agrikole_Typography', 'customize_preview_init' ) );
				add_action( 'wp_head', array( 'Agrikole_Typography', 'live_preview_styles' ), 999 );
			} else {
				add_filter( 'agrikole_custom_colors_css', array( 'Agrikole_Typography', 'head_css' ), 999 );
			}
		}
		...
		public static function elements() {
			// Set default font
			$body_default = 'Barlow';
			$heading_default = 'Amatic SC';

			// Return settings
			$array = apply_filters( 'agrikole_typography_settings', array(
				'body' => array(
					'label' => esc_html__( 'Body', 'agrikole' ),
					'target' => 'body',
					'defaults' => array(
				...
				'headings' => array(
					'label' => esc_html__( 'Headings', 'agrikole' ),
				...
				'woocommerce_product_title' => array(
					'label' => esc_html__( 'Woocommerce: Product Title', 'agrikole' ),
					'target' => '.products li h2',
					'exclude' => array( 'font-color' ),
					'active_callback' => 'agrikole_cac_has_woo',
				),
				'woocommerce_price' => array(
					'label' => esc_html__( 'Woocommerce: Price Amount', 'agrikole' ),
					'target' => '.products li .price',
					'exclude' => array( 'font-color' ),
					'active_callback' => 'agrikole_cac_has_woo',
				),
				'woocommerce_single_product_title' => array(
					'label' => esc_html__( 'Woocommerce: Single Product Title', 'agrikole' ),
					'target' => '.woo-single-post-class .summary h1',
					'exclude' => array( 'font-color' ),
					'active_callback' => 'agrikole_cac_has_woo',
				),
				'woocommerce_single_price' => array(
					'label' => esc_html__( 'Woocommerce: Single Price Amount', 'agrikole' ),
		...
		public static function customize_preview_init() {
			wp_enqueue_script( 'agrikole-typography-customize-preview',
				get_template_directory_uri() .'/framework/customizer/typography-customize.js',
				array( 'customize-preview' ),
				'1.0.0',
				true
			);
			wp_localize_script( 'agrikole-typography-customize-preview', 'agrikole', array(
		...
		public static function register ( $wp_customize ) {
			...
			// Add General Panel
			$wp_customize->add_panel( 'agrikole_typography', array(
				'priority' => 142,
				'capability' => 'edit_theme_options',
				'title' => esc_html__( 'Typography', 'agrikole' ),
			) );

			// Add General Tab
			$wp_customize->add_section( 'agrikole_typography_general' , array(
				'title' => esc_html__( 'General', 'agrikole' ),
				'priority' => 1,
				'panel' => 'agrikole_typography',
			) );
			...
			$wp_customize->add_control( new Agrikole_Customize_Multicheck_Control( $wp_customize, 'google_font_subsets', array(
				'label' => esc_html__( 'Font Subsets', 'agrikole' ),
				'section' => 'agrikole_typography_general',
			...
			foreach( $elements as $element => $array ) {
				...
				if ( $label ) {
					...
					// Define Section
					$wp_customize->add_section( 'agrikole_typography_'. $element , array(
						'title' => $label,
						'priority' => $count,
						'panel' => 'agrikole_typography',
					...
					if ( in_array( 'font-family', $attributes ) ) {
						...
						// Add Control
						$wp_customize->add_control( new Agrikole_Fonts_Dropdown_Custom_Control( $wp_customize, $element .'_typography[font-family]', array(
								'label' => esc_html__( 'Font Family', 'agrikole' ),
								'section' => 'agrikole_typography_'. $element,
					...
					if ( in_array( 'font-weight', $attributes ) ) {
						$wp_customize->add_setting( $element .'_typography[font-weight]', array(
							'type' => 'theme_mod',
							'description' => esc_html__( 'Note: Not all Fonts support every font weight style.', 'agrikole' ),
							'sanitize_callback' => false,
							'transport' => $transport,
						) );
						$wp_customize->add_control( $element .'_typography[font-weight]', array(
							'label' => esc_html__( 'Font Weight', 'agrikole' ),
							'section' => 'agrikole_typography_'. $element,
							...
							'choices' => array(
								...
								'100' => esc_html__( 'Extra Light: 100', 'agrikole' ),
								'200' => esc_html__( 'Light: 200', 'agrikole' ),
								'300' => esc_html__( 'Book: 300', 'agrikole' ),
								'400' => esc_html__( 'Normal: 400', 'agrikole' ),
								'500' => esc_html__( 'Medium: 500', 'agrikole' ),
								'600' => esc_html__( 'Semibold: 600', 'agrikole' ),
								'700' => esc_html__( 'Bold: 700', 'agrikole' ),
								'800' => esc_html__( 'Extra Bold: 800', 'agrikole' ),
								'900' => esc_html__( 'Black: 900', 'agrikole' ),
							),
							'description' => esc_html__( 'Important: Not all fonts support every font-weight.', 'agrikole' ),
						) );
					}
					...
					if ( in_array( 'font-style', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[font-style]', array(
							'label' => esc_html__( 'Font Style', 'agrikole' ),
							'section' => 'agrikole_typography_'. $element,
							...
							'choices' => array(
								'' => esc_html__( 'Default', 'agrikole' ),
								'normal' => esc_html__( 'Normal', 'agrikole' ),
								'italic' => esc_html__( 'Italic', 'agrikole' ),
							),
						) );
					}
					...
					if ( in_array( 'text-transform', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[text-transform]', array(
							'label' => esc_html__( 'Text Transform', 'agrikole' ),
							'section' => 'agrikole_typography_'. $element,
							...
							'choices' => array(
								'' => esc_html__( 'Default', 'agrikole' ),
								'capitalize' => esc_html__( 'Capitalize', 'agrikole' ),
								'lowercase' => esc_html__( 'Lowercase', 'agrikole' ),
								'uppercase' => esc_html__( 'Uppercase', 'agrikole' ),
							),
						) );
					}
					...
					if ( in_array( 'font-size', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[font-size]', array(
							'label' => esc_html__( 'Font Size', 'agrikole' ),
							'section' => 'agrikole_typography_'. $element,
							'settings' => $element .'_typography[font-size]',
							'priority' => 5,
							'type' => 'text',
							'description' => esc_html__( 'Value in px or em.', 'agrikole' ),
							'active_callback' => $active_callback,
						) );
					}
					...
					if ( in_array( 'font-color', $attributes ) ) {
						...
						$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, $element .'_typography_color', array(
							'label' => esc_html__( 'Font Color', 'agrikole' ),
							'section' => 'agrikole_typography_'. $element,
					...
					if ( in_array( 'line-height', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[line-height]',
							array(
								'label' => esc_html__( 'Line Height', 'agrikole' ),
								'section' => 'agrikole_typography_'. $element,
					...
					if ( in_array( 'letter-spacing', $attributes ) ) {
						...
						$wp_customize->add_control( new WP_Customize_Control( $wp_customize, $element .'_typography_letter_spacing', array(
							'label' => esc_html__( 'Letter Spacing', 'agrikole' ),
							'section' => 'agrikole_typography_'. $element,
		...
		public static function loop( $return = 'css' ) {
			...
			foreach( $elements as $element => $array ) {
				...
				$get_mod = agrikole_get_mod( $element .'_typography' );
		...
		public static function load_fonts() {
			...
			if ( ! empty( $fonts ) && is_array( $fonts ) ) {
				foreach ( $fonts as $font ) {
					agrikole_enqueue_google_font( $font );
				}
			}
