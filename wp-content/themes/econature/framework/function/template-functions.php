					...
					if ($cmsms_heading == 'custom') {
						...
						if (is_search()) {
							...
							if (!empty($wp_query->found_posts)) {
								...
								echo esc_html__('Search results for', 'econature') . ': ' . esc_attr(get_search_query());
							}
						} elseif (is_archive()) {
							if (is_day()) {
...
function cmsms_thumb_rollover($cmsms_id, $type = 'post-thumbnail', $rollover = true, $open_link = true, $group = false, $attachment_images = false, $attachment_video_type = false, $attachment_video_link = false, $attachment_video_links = false, $highImg = false, $show = true, $link_redirect = false, $link_url = false) {
	...
	if ($rollover) {
		if (
			...
			$out .= '<a ' . ($is_video_selfhosted ? $shv_out : 'href="' . ((!$attachment_video_link) ? $cmsms_image_link[0] : embedConvert($attachment_video_link)) . '"') . ' rel="ilightbox[' . $cmsms_id . '_' . $unique_id . ']" title="' . esc_attr($cmsms_title) . '" class="cmsms_image_link' . (($open_link) ? '' : ' no_open_link') . '"><span class="cmsms-icon-search-7"></span></a>';
		}
		
		
		if ($open_link) {
			$out .= '<a href="' . (($link_redirect == 'true' && $link_url != '') ? esc_url($link_url) : esc_url(get_permalink($cmsms_id))) . '" title="' . esc_attr($cmsms_title) . '" class="cmsms_open_link"><span class="cmsms-icon-attach-6"></span></a>';
		}
	} else {
...
function cmsms_sharing_box($title_box = false, $tag = 'h3') {
	...
	echo "<aside class=\"share_posts\">
		...
		<div class=\"share_posts_inner\">
			<a href=\"https://www.facebook.com/sharer/sharer.php?display=popup&u={$page_link}\">" . esc_html__('Facebook', 'econature') . "</a>
			<a href=\"https://twitter.com/intent/tweet?text=" . urlencode(html_entity_decode(sprintf(esc_attr__("Check out '%s' on %s website", 'econature'), $social_title, $website_name), ENT_QUOTES, 'UTF-8')) . "&url={$page_link}\">" . esc_html__('Twitter', 'econature') . "</a>
			<a href=\"https://pinterest.com/pin/create/button/?url={$page_link}&media={$pinterest_img}&description={$social_title}\">" . esc_html__('Pinterest', 'econature') . "</a>
		</div>
	</aside>
";
}
...
function cmsms_author_box($title_box = false, $tag = 'h3') {
	...
	if ($user_description) {
		...
		if ($user_email) {
			echo '<li>' . 
				'<a href="mailto:' . $user_email . '" class="cmsms-icon-mail-6" title="' . esc_attr__('Email', 'econature') . '" target="_blank"></a>' . 
			'</li>';
		}
		
		if ($user_url) {
			echo '<li>' . 
				'<a href="' . esc_url($user_url) . '" class="cmsms-icon-globe-1" title="' . esc_attr__('Website', 'econature') . '" target="_blank"></a>' . 
			'</li>';
		}
		
		if ($user_googleplus) {
			echo '<li>' . 
				'<a href="//plus.google.com/+' . $user_googleplus . '" class="cmsms-icon-gplus-3" title="' . esc_attr__('Website', 'econature') . '" target="_blank"></a>' . 
			'</li>';
		}
		
		if ($user_twitter) {
			echo '<li>' . 
				'<a href="//twitter.com/' . $user_twitter . '" class="cmsms-icon-twitter-5" title="' . esc_attr__('Twitter', 'econature') . '" target="_blank"></a>' . 
			'</li>';
		}
		
		if ($user_facebook) {
			echo '<li>' . 
				'<a href="' . esc_url($user_facebook) . '" class="cmsms-icon-facebook-5" title="' . esc_attr__('Facebook', 'econature') . '" target="_blank"></a>' . 
			'</li>';
		}
...
function cmsms_related($tag = 'h3', $related_box = false, $tgsarray = null, $popular_box = false, $recent_box = false, $related_number = 4, $type = 'post', $taxonomy = null) {
	if (($related_box && !empty($tgsarray)) || $recent_box || $popular_box) {
		...
		if ($type == 'post') {
			echo esc_html__('More posts', 'econature');
		} else {
			echo esc_html__('More projects', 'econature');
		}
		...
		if ($related_box && !empty($tgsarray) && $r->have_posts()) {
			echo '<li>' . 
				'<a href="#" class="current">' . esc_html__('Related', 'econature') . '</a>' . 
			'</li>';
		}
		...
		if ($popular_box) {
			...
			echo '">' . esc_html__('Popular', 'econature') . '</a>' . 
			'</li>';
		}
		...
		if ($recent_box) {
			...
			echo '">' . esc_html__('Latest', 'econature') . '</a>' . 
			'</li>';
		}
		...
		if ($related_box && !empty($tgsarray) && $r->have_posts()) {
			...
			if ($related->have_posts()) :
				...
				echo '<h5>';
				if ($type == 'post') {
					echo esc_html__('No related posts found', 'econature');
				} else {
					echo esc_html__('No related projects found', 'econature');
				}
				echo '</h5>';
			endif;
		...
		if ($popular_box) {
			...
			if ($popular->have_posts()) :
				...
				echo '<h5>';
				if ($type == 'post') {
					echo esc_html__('No popular posts found', 'econature');
				} else {
					echo esc_html__('No popular projects found', 'econature');
				}
				echo '</h5>';
			endif;
		...
		if ($recent_box) { 
			...
			if ($recent->have_posts()) :
				...
				echo '<h5>';
				if ($type == 'post') {
					echo esc_html__('No latest posts found', 'econature');
				} else {
					echo esc_html__('No latest projects found', 'econature');
				}
				echo '</h5>';
			endif;
