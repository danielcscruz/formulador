...
function cmsms_row($atts, $content = null) {
	...
	if ($data_bg_color_custom == 'true' || $data_bg_img != '') {
		...
		if ($data_bg_img != '') {
			...
			$out_style .= "\n\t" . 'background-image: url(' . ( isset( $new_bg_src[0] ) ? esc_url( $new_bg_src[0] ) : '' ) . '); ' . 
	...
	$out_start = '<div id="cmsms_row_' . $unique_id . '" class="cmsms_row cmsms_color_scheme_' . esc_attr($data_color) . 
...
function cmsms_column($atts, $content = null) {
    ...
    return cmsms_divpdel('<div class="cmsms_column ' . esc_attr($new_width) . 
	(($data_classes != '') ? ' ' . esc_attr($data_classes) : '') . 
	'"' . 
...
function cmsms_notice($atts, $content = null) {
    ...
    $out .= '<div id="cmsms_notice_' . $unique_id . '" class="cmsms_notice ' . esc_attr($type) . 
	(($icon != '') ? ' ' . esc_attr($icon) : '') . 
	(($classes != '') ? ' ' . esc_attr($classes) : '') . 
	'"' . 
	(($animation != '') ? ' data-animation="' . esc_attr($animation) . '"' : '') . 
...
function cmsms_icon_box($atts, $content = null) {
    ...
    $out .= '<div id="cmsms_icon_box_' . $unique_id . '" class="cmsms_icon_box ' . esc_attr($type) . 
	(($type != 'cmsms_box_heading') ? ' ' . esc_attr($box_icon) : '') . 
	(($classes != '') ? ' ' . esc_attr($classes) : '') . 
	'"' . 
	(($animation != '') ? ' data-animation="' . esc_attr($animation) . '"' : '') . 
	(($animation != '' && $animation_delay != '') ? ' data-delay="' . esc_attr($animation_delay) . '"' : '') . 
	'>' . "\n" . 
		'<div class="icon_box_inner">' . "\n" . 
			'<' . $heading_type . (($type == 'cmsms_box_heading') ? ' class="' . esc_attr($box_icon)  . '"' : '') . '>' . esc_html($title) . '</' . $heading_type . '>' . "\n" . 
	...
	if ($button_show == 'true') {
		$out .= '<a href="' . esc_url($button_link) . '" class="cmsms_button icon_box_button"' . 
...
function cmsms_featured_block($atts, $content = null) {
	...
	if ($button_show == 'true') {
		...
		'<a href="' . esc_url($button_link) . '" class="featured_block_button cmsms_button' . 
...
function cmsms_toggles($atts, $content = null) {
	...
	if ($sort == 'true') {
		$toggles_filter = '<div class="cmsms_toggles_filter">' . "\n\t" . 
			'<a href="#" data-key="all" title="' . esc_attr__('All', 'cmsms_content_composer') . '" class="current_filter">' . esc_html__('All', 'cmsms_content_composer') . '</a>' . "\n";
...
function cmsms_icon_list_items($atts, $content = null) {
	...
	'<ul id="cmsms_icon_list_items_' . $unique_id . '" class="cmsms_icon_list_items cmsms_icon_list_type_' . esc_attr($type) . ' cmsms_icon_list_pos_' . $position . ' cmsms_color_type_' . $items_color_type . 
	(($classes != '') ? ' ' . esc_attr($classes) : '') . 
	'"' . 
	(($animation != '') ? ' data-animation="' . esc_attr($animation) . '"' : '') . 
	(($animation != '' && $animation_delay != '') ? ' data-delay="' . esc_attr($animation_delay) . '"' : '') . 
...
function cmsms_icon_list_item($atts, $content = null) {
	...
	if ($list_type == 'block') {
		...
		'">' . "\n" . 
			'<div class="cmsms_icon_list_item_inner">' . "\n" . 
				'<div class="cmsms_icon_list_icon_wrap">' . "\n" . 
					'<span class="cmsms_icon_list_icon ' . esc_attr($icon) . '"></span>' . "\n" . 
				'</div>' . "\n" . 
				'<div class="cmsms_icon_list_item_content">' . "\n" . 
					'<' . $list_heading . ' class="cmsms_icon_list_item_title">' . esc_html($title) . '</' . $list_heading . '>' . "\n" . 
		...
		$out_inner .= '<li id="cmsms_icon_list_item_' . $unique_id . '" class="cmsms_icon_list_item ' . esc_attr($icon) . 
		(($classes != '') ? ' ' . esc_attr($classes) : '') . 
		'">' . esc_html($title) . '</li>';
	}
}
...
function cmsms_stats($atts, $content = null) {
	...
	return $style . 
	'<div class="cmsms_stats stats_mode_' . $mode . ' stats_type_' . esc_attr($type) . 
	(($classes != '') ? ' ' . esc_attr($classes) : '') . 
...
function cmsms_stat($atts, $content = null) {
	...
	return '<div class="cmsms_stat_wrap' . (($stats_mode == 'counters') ? $stats_count : '') . '">' . "\n" . 
		...
		(($stats_mode == 'counters' && $stats_type == 'circles' && $custom_colors == 'true') ? ' data-bar-color="' . esc_attr($bg_color) . '"' : '') . 
		'>' . "\n" . 
			...
			'">' . "\n" . 
				...
				'<span class="cmsms_stat_counter_wrap">' . "\n" . 
					(($stats_mode == 'counters' && $stats_type == 'numbers') ? '<span class="cmsms_stat_prefix">' . esc_html($value_prefix) . '</span>' : '') . 
					...
					(($stats_mode == 'counters' && $stats_type == 'numbers') ? '<span class="cmsms_stat_suffix">' . esc_html($value_suffix) . '</span>' . "\n" : '') . 
				'</span>' . "\n" . 
...
function cmsms_tr($atts, $content = null) {
	...
	$out .= '<tr' . 
		(($type != '') ? ' class="cmsms_table_row_' . esc_attr($type) . '"' : '') . 
	'>' . 
		do_shortcode($content) . 
	'</tr>';
...
function cmsms_divider($atts, $content = null) {
	...
	(($type == 'transparent') ? 'cl' : 'cmsms_divider ' . esc_attr($type)) . 
	(($classes != '') ? ' ' . esc_attr($classes) : '') . 
	'"></div>';
...
function cmsms_client($atts, $content = null) {
	...
	if ($logo != '') {
		...
		if ($link != '') {
			...
			(($classes != '') ? ' ' . esc_attr($classes) : '') . 
			'">' . "\n" . 
				'<a href="' . (($link != '') ? '' . esc_url($link) : '') . '" target="_blank">' .  
...
function cmsms_button($atts, $content = null) {
	...
	$out .= '<div id="cmsms_button_' . $unique_id . '" class="button_wrap">' . 
		'<a href="' . esc_url($button_link) . '" class="cmsms_button' . 
		(($button_icon != '') ? ' ' . esc_attr($button_icon) : '') . 
		(($classes != '') ? ' ' . esc_attr($classes) : '') . 
		'"' . 
		(($animation != '') ? ' data-animation="' . esc_attr($animation) . '"' : '') . 
		...
		if (is_numeric($new_image_thumb[0])) {
			...
			if ($link != '') {
				...
				'>' . "\n" . 
					...
					'>' . 
						'<img' .
							( isset( $new_image[0] ) ? ' src="' . esc_url( $new_image[0] ) . '"' : '' ) .
							( '' !== $caption ? ' alt="' . esc_attr( $caption ) . '"' : '' ) .
						' />' .
					'</a>' . "\n" . 
					(($caption != '') ? '<p class="cmsms_img_caption">' . esc_html($caption) . '</p>' : '') . 
				'</div>' . "\n";
			} else {
				$out .= '<div class="cmsms_img ' . $img_align . 
				(($caption != '') ? ' with_caption' : '') . 
				...
				$out .= '<li class="' . $new_gallery_col . '">' . 
					'<figure>';
						...
						if ($gallery_links != 'none') {
							...
							$out .= '<a' .
								( ( 'blank' === $gallery_links ) ? ' target="_blank"' : '' ) .
								( isset( $image_src[0] ) ? ' href="' . esc_url( $image_src[0] ) . '"' : '' ) .
								( ( 'lightbox' === $gallery_links ) ? ' rel="ilightbox[' . esc_attr( $unique_id ) . ']"' : '' ) .
								( '' !== $image_caption ? ' data-caption="' . esc_attr( $image_caption ) . '"' : '' ) .
							'>';
						}
	...
	if ($quote_mode == 'slider') {
		...
		'">' . "\n" . 
			'<article class="cmsms_quote_inner">' . "\n" . 
				...
				(($name != '') ? '<h6 class="quote_title">' . esc_html($name) . '</h6>' . "\n": '') . 
				(($subtitle != '') ? '<span class="quote_subtitle">' . esc_html($subtitle) . '</span>' . "\n": '');
				...
				if ($link != '' && $website != '') {
					$quote_out .= '<a class="quote_link" target="_blank" href="' . esc_url($link) . '">' . esc_html($website) . '</a>' . "\n";
				} elseif ($link == '' && $website != '') {
					$quote_out .= '<span class="quote_site">' . esc_html($website) . '</span>' . "\n";
				} elseif ($link != '' && $website == '') {
					$quote_out .= '<a class="quote_link" target="_blank" href="' . esc_url($link) . '">' . esc_url($link) . '</a>' . "\n";
				}
...
function cmsms_pricing_table_item($atts, $content = null) {
	...
	'>' . "\n" . 
		...
		'<span class="cmsms_currency">' . esc_html($currency) . '</span>' . "\n" . 
		'<span class="cmsms_price">' . esc_html($price) . '</span>' . "\n" . 
		(($coins != '') ? '<span class="cmsms_coins">.' . esc_html($coins) . '</span>' . "\n" : '') . 
		(($period != '') ? '<br /><span class="cmsms_period">' . esc_html($period) . '</span>' . "\n" : '') . 
		'</div>' . "\n";
		...
		if ($button_show == 'true') {
			$price_out .= '<a href="' . esc_url($button_link) . '" class="cmsms_button' . 
	...
	if ($facebook == 'true' || $twitter == 'true' || $pinterest == 'true') {
		...
		if ($twitter == 'true') {
			$out .= '<div class="share_wrap">' . "\n" . 
				'<a href="https://twitter.com/intent/tweet?text=' . urlencode(html_entity_decode(sprintf(esc_attr__("Check out '%s' on %s website", 'cmsms_content_composer'), $social_title, $website_name), ENT_QUOTES, 'UTF-8')) . '&url=' . $page_link . '" class="button cmsms-icon-twitter">' . esc_html__('Twitter', 'cmsms_content_composer') . '</a>' . "\n" . 
			'</div>' . "\n";
		}
		
		if ($pinterest == 'true') {
			$out .= '<div class="share_wrap">' . "\n" . 
				'<a href="https://www.pinterest.com/pin/create/button/" data-pin-do="buttonBookmark" data-pin-custom="true" class="button cmsms_pinterest_button cmsms-icon-pinterest">' . esc_html__('Pinterest', 'cmsms-content-composer') . '</a>' . "\n" . 
			'</div>' . "\n";
		}
		
		if ($facebook == 'true') {
			$out .= '<div class="share_wrap">' . "\n" . 
				'<a href="//www.facebook.com/sharer/sharer.php?display=popup&u=' . $page_link . '" class="button cmsms-icon-facebook">' . esc_html__('Facebook', 'cmsms_content_composer') . '</a>' . "\n" . 
			'</div>' . "\n";
		}
			...
			if ($tweets != '') {
				...
				echo '<div class="cmsms_notice cmsms_notice_error cmsms-icon-cancel-6">' . "\n" . 
					'<div class="notice_content">' . "\n" . 
						'<p>' . esc_html__('Please add your Twitter API keys', 'cmsmasters') . ', ' . '<a target="_blank" href="https://cmsmasters.net/twitter-functionality/">' . esc_html__('read more how', 'cmsmasters') . '</a></p>' . "\n" . 
					'</div>' . "\n" . 
				'</div>' . "\n";
			}
			...
			<div id=\"cmsms_owl_carousel_{$unique_id}\" class=\"" . 
				...
				while ($query->have_posts()) : $query->the_post();
					...
					if ($post_type == 'post') {
						if (get_post_format() != '') {
							$out .= '<div>' . 
								load_template_part('framework/post-type/posts-slider/blog/' . get_post_format()) . 
							'</div>';
						} else {
							$out .= '<div>' . 
								load_template_part('framework/post-type/posts-slider/blog/standard') . 
							'</div>';
						}
					} elseif ($post_type == 'project') {
						if (get_post_format() != '') {
							$out .= '<div>' . 
								load_template_part('framework/post-type/posts-slider/portfolio/' . get_post_format()) . 
							'</div>';
						} else {
							$out .= '<div>' . 
								load_template_part('framework/post-type/posts-slider/portfolio/standard') . 
							'</div>';
						}
					}
...
";
	...
	if ($query->have_posts()) : 
		while ($query->have_posts()) : $query->the_post();
			if ($layout == 'columns') {
				if (get_post_format() != '') {
					$out .= load_template_part('framework/post-type/blog/page/masonry/' . get_post_format());
				} else {
					$out .= load_template_part('framework/post-type/blog/page/masonry/standard');
				}
			} elseif ($layout == 'timeline') {
				if (get_post_format() != '') {
					$out .= load_template_part('framework/post-type/blog/page/timeline/' . get_post_format());
				} else {
					$out .= load_template_part('framework/post-type/blog/page/timeline/standard');
				}
			} else {
				if (get_post_format() != '') {
					$out .= load_template_part('framework/post-type/blog/page/default/' . get_post_format());
				} else {
					$out .= load_template_part('framework/post-type/blog/page/default/standard');
				}
			}
		endwhile;
...
";
			...
			<div class=\"cmsms_project_filter\">
				...
				if ($sorting != '') {
					$out .= "<div class=\"cmsms_project_sort_block\">
						<a href=\"#\" name=\"project_name\" title=\"" . esc_attr($sorting_name_text) . "\" class=\"button cmsms_project_sort_but" . (($orderby == 'name') ? " current" . (($order == 'DESC') ? " reversed" : "") : "") . "\">
							<span>" . esc_html($sorting_name_text) . "</span>
						</a>
						<a href=\"#\" name=\"project_date\" title=\"" . esc_attr($sorting_date_text) . "\" class=\"button cmsms_project_sort_but" . (($orderby == 'date') ? " current" . (($order == 'DESC') ? " reversed" : "") : "") . "\">
	...
	if ($query->have_posts()) : 
		while ($query->have_posts()) : $query->the_post();
			if ($layout == 'puzzle') {
				if (get_post_format() != '') {
					$out .= load_template_part('framework/post-type/portfolio/page/puzzle/' . get_post_format());
				} else {
					$out .= load_template_part('framework/post-type/portfolio/page/puzzle/standard');
				}
			} else {
				if (get_post_format() != '') {
					$out .= load_template_part('framework/post-type/portfolio/page/grid/' . get_post_format());
				} else {
					$out .= load_template_part('framework/post-type/portfolio/page/grid/standard');
				}
			}
		endwhile;
				...
				if ($layout == 'vertical') {
					$out .= '<div class="pl_content">' . "\n" . 
						...
						if ($cmsms_profile_subtitle != '') {
							$out .= '<h5 class="pl_subtitle">' . esc_html($cmsms_profile_subtitle) . '</h5>' . "\n";
						}
					...
					if (has_post_thumbnail()) {
						...
						$out .=  '<div class="pl_img"><a href="' . esc_url(get_permalink()) . '"><div class="pl_noimg cmsms-icon-user-1"></div></a></div>' . "\n";
					}
